...
namespace APINewsFeed.Controllers
{
    ...
    public class PostController : ControllerBase
    {
        ...
        public async Task<ActionResult> GetPostById([FromQuery] GetPostByIdDTO getPostByIdDTO)
        {
            ...
            if (post == null) return NotFound("Такого поста не существует");
            return Ok(post);
        }
        /// <summary>
        ...
        public async Task<ActionResult> GetPosts([FromQuery] FilterPostDTO filterPostDTO)
        {
            ...
            if (!posts.Any()) return NotFound("Постов не найдено");
            return Ok(posts);
        }
        /// <summary>
        ...
        public async Task<ActionResult> UpdatePost([FromForm] UpdatePostDTO updatePostDTO)
        {
            ...
            if (updatePost == null) return NotFound("Такого поста не существует");
        ...
        /// <response code="404">Пост с таким id не найден</response>
        [HttpDelete("delete")]
        public async Task<ActionResult> DeletePost([FromForm] DeletePostDTO deletePostDTO)
        {
            ...
            if (id == Guid.Empty) return NotFound("Пост с таким id не найден");
            return Ok(new { id = id });
        }
    }
}
